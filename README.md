...
Pages deploy. It triggers on the `deployment_status` event reaching `success` for the
`github-pages` environment, rather than on `push`, because a purge fired at push time
races the deploy and generally loses — evicting the cache while the origin still serves
the old build, which then gets cached again with a fresh TTL.

It originally used `workflow_run` against GitHub's "pages build and deployment"
workflow, which never fired: that trigger only matches workflows defined by a file in
`.github/workflows`, and the Pages build is generated by GitHub rather than committed
here. If you change this trigger, confirm it actually ran after the next deploy instead
of assuming it did — the failure is silent.
